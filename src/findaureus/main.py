...
class RawImageFile:
    ...
    def LowerValue_exp_theshold(self,input_image):
        '''
    ...
    def ValueForMask(self,input_image):
        '''
        ...
        try:
            lower = RawImageFile.LowerValue_exp_theshold(input_image)
        except ZeroDivisionError:
            lower=()
        
        return(lower, upper)

    def CreateBacteriaMask(self,input_image, lower, upper):
        '''
    ...
    def MorphologicalOperations(self,bacteria_mask):
        '''
    ...
    def ContourSelection (self,input_image,avaliable_contours):
        '''
    ...
    def FindingContours(self,input_morphed_image):
        '''
    ...
    def GetPixelWiseBacteriaCoordinates(self,input_morphed_image):
        '''
    ...
    def NonMaxSuppression(self,boxes, overlap_thresh):
        '''
    ...
    def MakeBoundingBoxWithCentroid(self,input_image, found_contour, image_file_object):
        '''
        ...
        selected_boxes = RawImageFile.NonMaxSuppression(boxes, overlap_thresh=0.3)
    ...
    def FindBacteriaAndNoBacteria(self,input_image_list, file_metadata):
        '''
        ...
        for imageno in range(0,len(input_image_list)):
            ...
            maskvalue_lower, maskvalue_upper = RawImageFile.ValueForMask(input_image)
            if maskvalue_lower == ():
                no_bac_image_list.append(input_image)
                no_bac_image_name_list.append('z'+str(imageno))
                continue
            mask_image = RawImageFile.CreateBacteriaMask(input_image, maskvalue_lower, maskvalue_upper)
            morph_image = RawImageFile.MorphologicalOperations(mask_image)
            contours_avaliable = RawImageFile.FindingContours(morph_image)
            bac_pixel_coordinates = RawImageFile.GetPixelWiseBacteriaCoordinates(morph_image)
            ...
            bac_image,bac_centroid_coordinates,bact_area = RawImageFile.MakeBoundingBoxWithCentroid(input_image, contours_avaliable, file_metadata)
...
class ReadFile():
    ...
    def ElementToDict(element):
        ...
        for child_element in element:
            child_dictionary = RawImageFile.ReadFile.ElementToDict(child_element)
        ...
        if image_file_path.endswith(".czi"):
            ...
            for chclr in range (0, len(channel_colours)):
                try:
                    ...
                    closest_name = RawImageFile.ClosestColour(rgb)
...
class ReadFileException:
    ...
    def ChannelImageList(image_file_numpy_array, user_option):
        ...
        input_image_list = []
        two_d_image = image_file_numpy_array
